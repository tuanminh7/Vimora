        ...
        <template v-if="$router.currentRoute.path !== '/login'">
            ...
            >
                ...
                <div id="logo" class="d-flex justify-center my-5" v-if="$vuetify.breakpoint.lgAndUp && !navbarCollapsed">
                    ...
                    <span class="brand-wordmark text--text"><span class="brand-mark">V</span><span class="brand-rest">imora</span></span>
                </div>
                ...
                <v-list nav shaped dense class="pl-0">
                    ...
                    <v-list-item class="navigation-button" @click="openLogoutDialog">
                        ...
                        <span class="pl-6"> {{ $t('common.logout') }} </span>
                    </v-list-item>
                </v-list>
                ...
                <template v-slot:append>
                    ...
                    <template v-if="!$vuetify.breakpoint.md && !navbarCollapsed">
                        <v-list nav shaped dense class="pl-0">
                            ...
                            <v-list-item class="navigation-button" @click="languageSelectionDialog = true;">
                                ...
                                <span class="pl-5"> {{ $t('common.language') }}</span>
                            </v-list-item>
                        </v-list>
                    </template>
                    ...
                    <template v-else>
                        <v-btn v-if="$vuetify.breakpoint.lgAndUp" id="collapse" rounded text class="mini-drawer-button" @click="expandNavbar" :title="$t('layout.expandSidebar')">
                            <v-icon>mdi-arrow-collapse-right</v-icon>
                        </v-btn>
                        <v-btn id="theme" rounded text class="mini-drawer-button" @click="themeSelectionDialog = true" :title="$t('common.theme')">
                            <v-icon>mdi-palette</v-icon>
                        </v-btn>
                        <v-btn id="language" rounded text class="mini-drawer-button" @click="languageSelectionDialog = true" :title="$t('layout.selectLanguage')">
    ...
    export default {
        data: function() {
            return {
                ...
                navigation: [
                    ...
                    {
                        name: this.$t('layout.library'),
                        url: '/books',
                        icon: 'mdi-bookshelf',
                        bottomNav: true,
                    },
                    {
                        name: this.$t('layout.vocabulary'),
                        url: '/vocabulary/search',
                        icon: 'mdi-translate',
                        bottomNav: true,
                    },
                    {
                        name: this.$t('layout.review'),
                        url: '',
                    ...
                    {
                        name: this.$t('layout.userSettings'),
                        url: '/user-settings',
                        icon: 'mdi-account-cog',
                        bottomNav: false,
                    },
                    {
                        name: this.$t('layout.userManual'),
                        url: '/user-manual',
        ...
        beforeMount() {
            ...
            if(this.$store.getters['shared/userAdmin']) {
                this.navigation.push({
                    name: this.$t('layout.adminSettings'),
                    url: '/admin',
        ...
        methods: {
            ...
            navigationClick(itemName, event) {
                if (itemName === this.$t('layout.review')) {
                    this.startReviewDialog = true;
                    event.preventDefault();
                }

                // clicked on user manual
                if (itemName === this.$t('layout.userManual') && this.$router.currentRoute.path !== '/user-manual') {
