<template>
    <v-dialog content-class="language-selection-dialog" v-model="value" scrollable persistent>
        <v-card class="rounded-lg" :loading="loading">
            <v-card-title>
                <span class="text-h5">{{ $t('dialogs.languageTitle') }}</span>
                <v-spacer></v-spacer>
                    ...
                    <v-row align="center">
                        <v-col class="grow">
                            <template v-if="notInstalledLanguages === 1">
                                {{ $t('dialogs.oneAdditionalLanguage') }}
                            </template>

                            <template v-else>
                                {{ $t('dialogs.manyAdditionalLanguages', '', { count: notInstalledLanguages }) }}
                            </template>

                            <template v-if="!$store.getters['shared/userAdmin']">
                                {{ $t('dialogs.adminOnlyLanguages') }}
                            </template>
                        </v-col>
