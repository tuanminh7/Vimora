    ...
    <v-dialog v-model="value" persistent max-width="500px" height="300px" :loading="loading">
        <v-card class="rounded-lg">
            <v-card-title>
                ...
                <span class="text-h5">{{ $t('dialogs.logoutTitle') }}</span>
                <v-spacer></v-spacer>
            ...
            <v-card-text class="pt-4 pb-6">
                {{ $t('dialogs.logoutConfirm') }}
            </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn rounded text @click="close">{{ $t('common.cancel') }}</v-btn>
                <v-btn rounded text @click="logout"><v-icon class="mr-1">mdi-logout</v-icon>{{ $t('common.logout') }}</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
